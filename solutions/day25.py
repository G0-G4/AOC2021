...
INP = os.path.join(INPUTS, 'input25.txt')


def step(map):
    rows, cols = len(map), len(map[0])
    old = copy.deepcopy(map)
    moved = set()
    for i in range(rows):
        for j in range(cols): 
            if map[i][j] == '>' and old[i][(j + 1) % cols] == '.' and (i, j) not in moved:
                map[i][j] = '.'
                map[i][(j + 1) % cols] = '>'
                moved.add((i, (j + 1) % cols))
    # print(*map, sep='\n')
    # print()
    ...
    for i in range(rows):
        for j in range(cols): 
            if map[i][j] == 'v' and old[(i + 1) % rows][j] == '.' and (i, j) not in moved:
                map[i][j] = '.'
                map[(i + 1) % rows][j] = 'v'
                moved.add(((i + 1) % rows, j))
    # print(*map, sep='\n')
    # print()
